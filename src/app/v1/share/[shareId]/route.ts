...
// ONE share-resolution route (was two conflicting dynamic segments: [shareId] +
// [slug]). `shareId` is the frontend's link param. Resolve the rich share-by-code
// first (board / website / pixel / link + white-label); fall back to the legacy
// website-only shared id for back-compat.
export async function GET(_request: Request, { params }: { params: Promise<{ shareId: string }> }) {
  const { shareId } = await params;

  const share = await getShareByCode(shareId);

  if (!share) {
    // Legacy: an old website-only shared id.
    const website = await getSharedWebsite(shareId);
    if (!website) {
      return notFound();
    }
    const legacy = { websiteId: website.id };
    return json({ ...legacy, token: createToken(legacy, secret()) });
  }
