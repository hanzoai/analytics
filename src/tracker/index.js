(window => {
  ...
  const initProviders = () => {
    ...
    // Hanzo Insights — load Insights-compatible SDK on demand
    if (insightsKey) {
      // biome-ignore lint: vendored Insights snippet — minified third-party code
      !(function (t, e) {
        var o, n, p, r;
        e.__SV ||
          ((window.insights = e),
          ...
          (e.__SV = 1));
      })(document, window.insights || []);
      window.insights.init(insightsKey, {
        api_host: insightsHost,
        person_profiles: 'identified_only',
        capture_pageview: false,
        capture_pageleave: true,
      });
      // Also expose as window.hi for @hanzo/insights compat
      window.hi = window.insights;
    }
  };
  ...
  const forwardPageView = (url, title) => {
    ...
    if (insightsKey && window.insights) {
      window.insights.capture('$pageview', { $current_url: url, title });
    }
  };
  ...
  const forwardEvent = (name, data) => {
    ...
    if (insightsKey && window.insights) {
      window.insights.capture(name, data || {});
    }
  };
  ...
  const sendAST = async payload => {
    ...
    if (insightsKey && window.insights) {
      try {
        window.insights.capture('$ast_collected', payload);
      } catch {}
    }
  };
  ...
  const identify = (id, data) => {
    if (typeof id === 'string') {
      ...
      if (insightsKey && window.insights) {
        window.insights.identify(id, typeof data === 'object' ? data : {});
      }
    }
