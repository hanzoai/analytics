(window => {
  ...
  const eventRegex = /data-analytics-event-([\w-_]+)/;
  const eventNameAttribute = _data + 'analytics-event';
  ...
  const trackingDisabled = () =>
    disabled ||
    !website ||
    (localStorage && localStorage.getItem('analytics.disabled')) ||
    ...
    try {
      const res = await fetch(endpoint, {
        ...
        headers: {
          ...
          ...(typeof cache !== 'undefined' && { 'x-analytics-cache': cache }),
        },
        credentials: 'omit',
      });
